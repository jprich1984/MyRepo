...
#sorted_words= $(curl  $1 |tr [A-Z] [a-z]|grep -oE "\w+"|sort|uniq -c|sort -k1n,1 -k2r,2r)
sorted_words=$(curl -s $1|tr [A-Z] [a-z]|grep -oE "\w+"|sort|uniq -c|sort -k1n,1 -k2r,2r)
echo -e "${sorted_words}"
...
parity=echo -e "($total_uniq_words%2)"
should_be=0
echo " $parity"
if [$parity -eq 0]]
 then
	declare -i x=$total_uniq_words mid1=x/2
	declare -i  x==$mid1 mid2=x+1
	declare -i x=$mid1 y=$mid2 median_pos=(x+y)/2

	echo "Parity ${parity}, Mid1: $mid1, Mid2: $mid2"
 else
	declare -i x=$total_uniq_words mid1=x/2
	declare -i x=$mid1  median_pos=x+1
	echo "Parity ${parity}, Mid1: $mid1"
fi
